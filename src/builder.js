...
const createSVGNode = (name) => {
  return document.createElementNS(svgNS, name)
}

const addMediumFontFamily = (text) => {
  text.setAttribute("font-family", "Roboto-Medium");
}

const addBoldFontFamily = (text) => {
...
const createDayElement = (label, tags, x, y, color1, color2, color3, height) => {
  const g = createSVGNode("g");
  const rect = createSVGNode("rect");
  rect.setAttribute("width", 134)
  rect.setAttribute("height", height ? height : 134)
  rect.setAttribute("stroke", color1);
  rect.setAttribute("x", x);
  rect.setAttribute("y", y);

  g.append(rect);
  g.append(createDayHeader(label, x, y, color2, color3));
  if (tags && tags.length > 0) g.append(createDayTags(tags, x, y));
  return g;
}

const createWorkDay = (label, tags, x, y, height) => {
  return createDayElement(label, tags, x, y, "#D0D0D0", "#D0D0D0", "#5F5F5F", height);
}

const createDayOff = (label, tags, x, y, height) => {
  return createDayElement(label, tags, x, y, "#F4F4F4", "#B8B8B8", "#373737", height);
}

const createDoubleDay = (day, x, y) => {
  const g = createSVGNode("g");
  let createFunction = createDayOff;
  if (day.type == "work-day") {
    createFunction = createWorkDay;
  }
  g.append(createFunction(day.label, null, x, y, 65));
  g.append(createFunction(day.otherDay.label, null, x, y + 70, 64));
  return g;
}
...
const createBirthdayTag = (x, y) => {
  const g = createSVGNode("g");
  const path1 = createSVGNode("path");
  ...
  const path2 = createSVGNode("path");
  ...
  const path3 = createSVGNode("path");
...
const createDay = (day, x, y) => {
  if (day.doubleDay) {
    return createDoubleDay(day, x, y);
  }
  switch(day.type) {
    case "day-off":
